...
void afficherStats(const Stats& S) {
    ...
    // cout << "---- ATMO (arrondi) ----" << endl;
    // for (const auto& pair : S.atmo) {
    //     cout << pair.first << " : " << pair.second << endl;
    // }
...
void mainTom(){
    ...
    // cout<<test<<endl;
    // Model model;
    // vector<Measurement> result = model.getMeasurements("Sensor0",100,"O3");
    // for (vector<Measurement>::iterator it = result.begin(); it != result.end(); ++it)
    //     {
    //         cout << "Timestamp : " << it->getTimestamp()
    //              << " | Attribut : " << it->getAttributeId()
    //              << " | Valeur : " << it->getValue() << endl;
    //     }
    Model model;
    Date Ddepar = Date("2019-01-01 12:00:00");
